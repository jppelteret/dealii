...
// This header replicates step-44, leaves some aspects of its implementation
...
namespace Step44
{
  ...
  public:
    ...
    {
      // using namespace SD;
      return c_1 * (trace(C_bar) - NumberType(dim));
    }
    template <typename NumberType>
    NumberType
    get_Psi_vol(const NumberType &J_tilde)
    {
      // using namespace SD;
      return (kappa / 4.0) *
             (J_tilde * J_tilde - NumberType(1.0) - 2.0 * log(J_tilde));
    }
  ...
  protected:
    ...
    virtual void
    assemble_system(const BlockVector<double> &solution_delta) = 0;
    void
    assemble_sc();
    void
  ...
  template <int dim>
  struct Step44_Base<dim>::PerTaskData_SC
  {
  ...
  template <int dim>
  struct Step44_Base<dim>::ScratchData_SC
  {
    void
    reset()
    {}
  ...
  template <int dim>
  struct Step44_Base<dim>::PerTaskData_UQPH
  {
    void
    reset()
    {}
  ...
  template <int dim>
  struct Step44_Base<dim>::ScratchData_UQPH
  {
  ...
  template <int dim>
  void
  Step44_Base<dim>::determine_component_extractors()
  {
    ...
    const BlockVector<double> &solution_delta)
  {
    ...
    WorkStream::run(dof_handler_ref.begin_active(),
                    dof_handler_ref.end(),
                    *this,
                    &Step44_Base::update_qph_incremental_one_cell,
                    &Step44_Base::copy_local_to_global_UQPH,
                    scratch_data_UQPH,
                    per_task_data_UQPH);
  ...
  template <int dim>
  void
  Step44_Base<dim>::update_qph_incremental_one_cell(
    const typename DoFHandler<dim>::active_cell_iterator &cell,
    ScratchData_UQPH &                                    scratch,
    PerTaskData_UQPH & /*data*/)
  {
  ...
  template <int dim>
  void
  Step44_Base<dim>::print_conv_header()
  {
  ...
  template <int dim>
  void
  Step44_Base<dim>::print_conv_footer()
  {
    static const unsigned int l_width = 155;
    for (unsigned int i = 0; i < l_width; ++i)
      std::cout << "_";
  ...
  template <int dim>
  double
  Step44_Base<dim>::compute_vol_current() const
  {
  ...
  template <int dim>
  std::pair<double, double>
  Step44_Base<dim>::get_error_dilation() const
  {
  ...
  template <int dim>
  void
  Step44_Base<dim>::get_error_residual(Errors &error_residual)
  {
  ...
  template <int dim>
  BlockVector<double>
  Step44_Base<dim>::get_total_solution(
  ...
  template <int dim>
  void
  Step44_Base<dim>::make_constraints(const int &it_nr)
  {
  ...
  Step44_Base<dim>::assemble_sc()
  {
    ...
    WorkStream::run(dof_handler_ref.begin_active(),
                    dof_handler_ref.end(),
                    *this,
                    &Step44_Base::assemble_sc_one_cell,
                    &Step44_Base::copy_local_to_global_sc,
                    scratch_data,
                    per_task_data);
    timer.leave_subsection();
  }
  template <int dim>
  void
  Step44_Base<dim>::copy_local_to_global_sc(const PerTaskData_SC &data)
  {
  ...
  template <int dim>
  void
  Step44_Base<dim>::assemble_sc_one_cell(
    const typename DoFHandler<dim>::active_cell_iterator &cell,
    ScratchData_SC &                                      scratch,
    PerTaskData_SC &                                      data)
  {
  ...
  template <int dim>
  std::pair<unsigned int, double>
  Step44_Base<dim>::solve_linear_system(BlockVector<double> &newton_update)
  ...
  template <int dim>
  void
  Step44_Base<dim>::output_results() const
  {
