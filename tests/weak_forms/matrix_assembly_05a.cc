...
// - Curl-curl matrix (vector-valued finite element)
//
// This test is derived from tests/weak_forms/matrix_assembly_04a.cc
...
run()
{
        ...
        for (const unsigned int q : fe_values.quadrature_point_indices())
          for (const unsigned int i : fe_values.dof_indices())
            for (const unsigned int j : fe_values.dof_indices())
              cell_matrix(i, j) += fe_values[field].curl(i, q) *
                                   fe_values[field].curl(j, q) *
                                   fe_values.JxW(q);
  ...
  // Scalar valued coefficient
  {
    ...
    assembler += bilinear_form(test_curl, coeff_func, trial_curl).dV();
  ...
  // Tensor valued coefficient (rank-0)
  {
    using namespace WeakForms;

    deallog << "Weak form assembly (bilinear form, rank-0 tensor valued coefficient)"
              << std::endl;
    system_matrix_wf = 0;
  ...
  // Tensor valued coefficient (rank-2)
  {
    using namespace WeakForms;

    deallog << "Weak form assembly (bilinear form, rank-2 tensor valued coefficient)"
              << std::endl;
    system_matrix_wf = 0;
