...
// - Third derivative-third derivative matrix (vector-valued finite element)
//
...
run()
{
  ...
  GridGenerator::subdivided_hyper_cube(triangulation, 2, 0.0, 1.0);
  ...
  const UpdateFlags update_flags = update_3rd_derivatives | update_JxW_values;
        ...
        for (const unsigned int q : fe_values.quadrature_point_indices())
          for (const unsigned int i : fe_values.dof_indices())
            for (const unsigned int j : fe_values.dof_indices())
              cell_matrix(i, j) += scalar_product(fe_values[field].third_derivative(i, q),
                                   fe_values[field].third_derivative(j, q)) *
                                   fe_values.JxW(q);
  ...
  // Scalar valued coefficient
  {
    ...
    assembler += bilinear_form(test_3rd_derivative, coeff_func, trial_3rd_derivative).dV();
