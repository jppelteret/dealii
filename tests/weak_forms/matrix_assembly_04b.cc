...
// - Div-div matrix (tensor-valued finite element)
//
...
run()
{
  ...
  const FESystem<dim, spacedim>  fe(FE_Q<dim, spacedim>(1), Tensor<2,dim>::n_independent_components);
...

  {
    ...
    FEValuesExtractors::Tensor<2> field (0);
        ...
        for (const unsigned int q : fe_values.quadrature_point_indices())
          for (const unsigned int i : fe_values.dof_indices())
            for (const unsigned int j : fe_values.dof_indices())
              cell_matrix(i, j) += fe_values[field].divergence(i, q) *
                                   fe_values[field].divergence(j, q) *
                                   fe_values.JxW(q);
  ...
  // Scalar valued coefficient
  {
    ...
    const SubSpaceExtractors::Tensor<2> subspace_extractor(0,"T","\\mathbf{T}");
    ...
    assembler += bilinear_form(test_div, coeff_func, trial_div).dV();
