...
// - Symmetric Laplace matrix (vector-valued finite element)
//
// This test is derived from tests/weak_forms/matrix_assembly_02b.cc
...
run()
{
  ...
  auto verify_assembly = [](const SparseMatrix<double> &system_matrix_std,
    ...
    Assert(system_matrix_wf.m() == system_matrix_std.m(), 
           ExcDimensionMismatch(system_matrix_wf.m(), system_matrix_std.m()));
    Assert(system_matrix_wf.n() == system_matrix_std.n(), 
           ExcDimensionMismatch(system_matrix_wf.n(), system_matrix_std.n()));
        ...
        for (const unsigned int q : fe_values.quadrature_point_indices())
          for (const unsigned int i : fe_values.dof_indices())
            for (const unsigned int j : fe_values.dof_indices())
              cell_matrix(i, j) += fe_values[field].symmetric_gradient(i, q) *
                                   fe_values[field].symmetric_gradient(j, q) *
                                   fe_values.JxW(q);
  ...
  // Scalar valued coefficient
  {
    ...
    const auto test_grad  = symmetric_gradient(test_ss);  // Shape function symmetric gradient
    const auto trial_grad = symmetric_gradient(trial_ss); // Shape function symmetric gradient
  ...
  // Tensor valued coefficient
  {
    ...
    const auto test_grad  = symmetric_gradient(test_ss);  // Shape function gradient
    const auto trial_grad = symmetric_gradient(trial_ss); // Shape function gradient
    const auto coeff_func = value<double>(coeff, [](const unsigned int) {
