...
// - Hessian-Hessian matrix (vector-valued finite element)
//
...
run()
{
  ...
  // Scalar valued coefficient
  {
    ...
    const auto coeff_func =
      value<double,spacedun>(coeff, [](const FEValuesBase<dim, spacedim> &, const unsigned int) { return 1.0; });

    // Still no concrete definitions
    MatrixBasedAssembler<dim, spacedim> assembler;
    assembler += bilinear_form(test_hess, coeff_func, trial_hess).dV();
